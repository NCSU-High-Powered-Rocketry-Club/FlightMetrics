...
class RangeSlider(LinearRegionItem):
    """A LinearRegionItem-based range slider."""

    rangeChanged = pyqtSignal(float, float)

    def __init__(self, min_value=0, max_value=100, start_min=20, start_max=80):
        super().__init__([start_min, start_max], movable=True)
    ...
    def _on_region_changed(self):
        """Handles when the LinearRegionItem is moved."""
        low, high = self.getRegion()
        self.low_value = max(self.min_value, low)
        self.high_value = min(self.max_value, high)
        self.rangeChanged.emit(self.low_value, self.high_value)

    def set_range(self, datasets):
        """Update the range based on dataset lengths."""
        lengths = [len(df) for df in datasets]
        self.max_value = max(lengths) if lengths else 1
        self.high_value = min(self.high_value, self.max_value)
        if self.high_value == 1:
            self.high_value = self.max_value
        self.setRegion([self.low_value, self.high_value])
        self.rangeChanged.emit(self.low_value, self.high_value)

    def state_button_update(self, low: int, high: int):
        """Update the region based on external input."""
        if low == 0 and high == 0:
            low, high = self.min_value, self.max_value

        self.low_value = low
        self.high_value = high
        self.setRegion([low, high])
        self.rangeChanged.emit(low, high)
